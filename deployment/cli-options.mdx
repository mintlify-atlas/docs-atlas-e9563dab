...
Specifies the directory from which slideshow JSON files are loaded. The path is resolved to an absolute path at startup. If the directory does not exist, the server will fall back to the bundled `data/slideshows/` directory inside the extracted archive.
